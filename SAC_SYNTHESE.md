...
Ce document décrit le pipeline SAC effectivement présent dans ce dépôt : une tâche MuJoCo d'insertion d'un pion dans un logement, pilotée par Stable-Baselines3.
...
La politique produit quatre valeurs normalisées dans `[-1, 1]` :
...
L'observation `float32` contient **32 valeurs** :

| Bloc | Taille | Échelle appliquée |
|---|---:|---|
| Positions articulaires | 4 | centrées et divisées par la demi-course |
| Vitesses articulaires | 4 | `/ [0.20, 0.20, 0.20, 2.0]` |
| Position effecteur | 3 | `/ 0.20 m` |
| Quaternion effecteur | 4 | aucune |
| Position pion relative au logement | 3 | `/ 0.05 m` |
| Quaternion relatif | 4 | aucune |
| Force/couple poignet | 6 | `/ [50, 50, 50, 5, 5, 5]` |
| Action précédente | 4 | déjà normalisée |
...
À chaque épisode, X/Y sont aujourd'hui tirés dans `[-0.2, 0.2] m`, Z vaut 0, et le yaw est tiré dans `[-90°, 90°]`. Les vitesses sont remises à zéro, puis 30 pas MuJoCo stabilisent l'état et servent à tarer le capteur F/T.

L'objectif est une pose relative `([0, 0, -0.045], quaternion identité)`. Un épisode dure au plus 250 décisions et s'arrête aussi en cas de :

- succès : erreur latérale < 1,5 mm, erreur de profondeur < 2 mm et rotation < 2° ;
...
+ 0.10 après une première descente (z relatif < -5 mm)
